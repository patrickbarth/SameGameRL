...
class View:
    ...
    def draw_board(self, screen, board):
        for row in range(NUM_ROWS):
            for col in range(NUM_COLS):
                color = board[row][col]
                self.tiles.append(Tile(col * TILE_SIZE + GAP, row * TILE_SIZE + GAP, TILE_SIZE, color, self.game, row, col))
    ...
    def show_score(self, screen, score):
        ...
        text = font.render("Game over!", True, (255, 255, 255))  # Text, antialiasing, and text color
        text_rect = text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 50 - 50))
        result_text = font.render(str(score[0]) + " - " + str(score[1]), True, (255, 255, 255))
        result_text_rect = result_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 50 - 50))
